...
def load_encode_image(image_path):
...
def load_student_images(dataset_folder):
    ...
    with Pool(cpu_count()) as p:
        encodings = p.map(load_encode_image, image_paths)
    for idx, encoding in enumerate(encodings):
        if encoding is not None:
            folder_name = os.path.basename(os.path.dirname(image_paths[idx]))
            name = os.path.splitext(os.path.basename(image_paths[idx]))[0]
